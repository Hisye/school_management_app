...
class UpdateStudentResult extends StatefulWidget {
  final Student student;
  final Result result;

  const UpdateStudentResult(
      {super.key, required this.student, required this.result});

  @override
  State<UpdateStudentResult> createState() => _UpdateStudentResultState();
}

class _UpdateStudentResultState extends State<UpdateStudentResult> {
  final _formKey = GlobalKey<FormState>();
  late String _subjectGrade;
  late String _subjectType;
  String adminPrefix = FirebaseHelper.getAdminPrefix();

  @override
  void initState() {
    super.initState();
    _subjectType = widget.result.subject;
    _subjectGrade = widget.result.grade;
  }
  ...
  Future<bool> _updateData() async {
    if (_formKey.currentState!.validate()) {
      ...
      final url = Uri.https(
          ...
          '/$adminPrefix/student-data/${widget.student.id}/result/${widget.result.id}.json');
      try {
        ...
        print('Description: $_subjectGrade');
        print('Date: $_subjectType');
        if (!context.mounted) {
          return true;
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Subject result update successfully'),
            ),
          );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Cannot update result'),
          ),
        );
